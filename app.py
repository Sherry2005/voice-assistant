...
.status-error   { background: #201515; color: #e05555; border: 1px solid #3a2020; }
...
.level-a1 { background: #1e2a20; color: #4CAF8A; border: 1px solid #2a3a34; }
.level-a2 { background: #1e2520; color: #6dbf9a; border: 1px solid #2a3530; }
.level-b1 { background: #1e2020; color: #c9a84c; border: 1px solid #2a3020; }
.level-b2 { background: #201e1a; color: #d4956a; border: 1px solid #302a20; }
.level-c1 { background: #201a1a; color: #e05555; border: 1px solid #302020; }
...
if "eleven_key" not in st.session_state:
    st.session_state.eleven_key = ""

# ── German tutor personas ──────────────────────────────────────────────────────
PERSONAS = {
...
3. If the user spoke German (even badly), gently correct ONE key mistake max. Format it as: "✏️ Korrektur: [wrong] → [correct] ([explanation in English])"
...
6. Be warm, encouraging, use simple everyday topics.""",
...
2. If the user made grammar mistakes, correct up to 2 mistakes. Format: "✏️ Korrektur: [wrong] → [correct]"
3. Push the learner to use more complex structures: Konjunktiv II, Perfekt, subordinate clauses.
4. If the user spoke English, respond in German and say (in German) that you'd like them to try in German.
5. Keep responses conversational, 2-4 sentences. This is spoken practice.""",

    "🔴 Advanced (C1)": """You are a sophisticated German conversation partner for advanced learners.

RULES:
1. Speak exclusively in natural, nuanced German — no hand-holding.
2. Correct grammar/style errors subtly woven into your reply (rephrase what they said correctly, naturally).
3. Use idiomatic expressions, regional flavor, complex sentence structures.
4. Engage deeply with topics: philosophy, culture, current events, abstract ideas.
5. If the user speaks English, respond in German only and do not acknowledge the English.
6. 3-5 sentences per response.""",
}

# ElevenLabs German voices
GERMAN_VOICES = {
    "Anna (Female, warm)": "Xb7hH8MSUJpSbSDYk0k2",        # multilingual female
    "Daniel (Male, deep)": "onwK4e9ZLuTAKqWW03F9",         # multilingual male  
    "Lena (Female, crisp)": "EXAVITQu4vr4xnSDxMaL",       # Sarah - clear female
    "Klaus (Male, authoritative)": "VR6AewLTigWG4xSOukaG", # Arnold - deep male
}
...
with st.sidebar:
    ...
    if "ELEVENLABS_API_KEY" in st.secrets:
        st.session_state.eleven_key = st.secrets["ELEVENLABS_API_KEY"]
        st.markdown('<span class="status-badge status-ready">● ElevenLabs loaded</span>', unsafe_allow_html=True)
    else:
        el_key = st.text_input("ElevenLabs API Key", type="password", placeholder="sk_...")
        if el_key:
            st.session_state.eleven_key = el_key
            st.markdown('<span class="status-badge status-ready">● ElevenLabs ready</span>', unsafe_allow_html=True)
        else:
            st.markdown('<span class="status-badge status-idle">○ ElevenLabs key needed</span>', unsafe_allow_html=True)
    ...
    speaking_rate = st.slider("Speaking Speed", 0.7, 1.2, 1.0, 0.05,
    ...
    try:
        with open(tmp_path, "rb") as audio_file:
            result = client.audio.transcriptions.create(
                ...
                language="de",  # hint: German — improves accuracy massively
            )
        return result.strip()
    finally:
        os.unlink(tmp_path)
...
def synthesize_elevenlabs(text: str, voice_id: str, eleven_key: str, speed: float) -> bytes:
    """Use ElevenLabs for native-quality German TTS."""
    client = ElevenLabs(api_key=eleven_key)

    # Strip correction lines from TTS — don't speak the ✏️ correction
    lines = text.split('\n')
    tts_lines = [l for l in lines if not l.strip().startswith('✏️')]
    tts_text = '\n'.join(tts_lines).strip()

    audio_generator = client.text_to_speech.convert(
        voice_id=voice_id,
        text=tts_text,
        model_id="eleven_multilingual_v2",  # best model for German
...
def render_message(role: str, content: str):
    """Render a chat bubble, separating correction lines visually."""
    lines = content.split('\n')
    main_lines = [l for l in lines if not l.strip().startswith('✏️')]
    correction_lines = [l for l in lines if l.strip().startswith('✏️')]
...
if audio_input is not None:
    ...
    if not st.session_state.eleven_key:
        missing.append("ElevenLabs API key")

    if missing:
        st.error(f"Please enter your {' and '.join(missing)} in the sidebar.")
    else:
        audio_bytes = audio_input.read()

        # Step 1: Transcribe (with German language hint)
        ...
        if not user_text:
            st.warning("Nichts gehört. Bitte nochmal versuchen! (No speech detected. Try again.)")
            st.stop()

        render_message("user", user_text)

        # Step 2: LLM reply (German tutor)
        ...
        # Step 3: ElevenLabs TTS — native German
        with st.spinner("Spreche... (Speaking)"):
            try:
                speech_bytes = synthesize_elevenlabs(
                    reply,
                    voice_id,
                    st.session_state.eleven_key,
                    speaking_rate,
                ...
                st.warning(f"TTS failed (reply shown above): {e}")
